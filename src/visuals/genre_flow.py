...
class ConsistentFeatureEngineering:
    """Thin wrapper around the shared FeaturePipeline for backward compatibility."""

    def __init__(self):
        self._pipeline = FeaturePipeline()

    def engineer_features(self, df):
        features, _ = self._pipeline.extract_features(df)
        return self._pipeline.engineer(features)

    def apply_scaling(self, features):
        return self._pipeline.scale(features, method='ensemble')

    def select_features(self, features, labels, n_features=100):
        return self._pipeline.select_features(features, labels, n_features=n_features)
