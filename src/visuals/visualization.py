...
# Unified figure output directory
FIG_DIR = 'paper/SMC/figs/pipeline'
...
def plot_validation_metrics(results, features_scaled=None, y_true=None, save=True):
    ...
    # Normalize metrics to [0, 1] for comparison (with divide-by-zero guards)
    sil_range = np.ptp(metrics['silhouette'])
    silhouette_norm = (metrics['silhouette'] - np.min(metrics['silhouette'])) / sil_range if sil_range > 0 else np.full_like(metrics['silhouette'], 0.5)
    
    # Invert and normalize inertia (lower is better)
    inertia_range = np.ptp(metrics['inertia'])
    inertia_norm = 1 - (metrics['inertia'] - np.min(metrics['inertia'])) / inertia_range if inertia_range > 0 else np.full_like(metrics['inertia'], 0.5)
    
    # Normalize Calinski-Harabasz
    cal_range = np.ptp(metrics['calinski'])
    calinski_norm = (metrics['calinski'] - np.min(metrics['calinski'])) / cal_range if cal_range > 0 else np.full_like(metrics['calinski'], 0.5)
    
    # Invert and normalize Davies-Bouldin (lower is better)
    db_range = np.ptp(metrics['davies_bouldin'])
    davies_norm = 1 - (metrics['davies_bouldin'] - np.min(metrics['davies_bouldin'])) / db_range if db_range > 0 else np.full_like(metrics['davies_bouldin'], 0.5)
    ...
    if save:
        save_path = os.path.join(FIG_DIR, 'validation_metrics_combined.png')
...
def plot_tsne_comparison(X, natural_labels, forced_labels, y_true, save=True):
    ...
    if save:
        save_path = os.path.join(FIG_DIR, 'tsne_comparison_interpretable.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Saved: {save_path}")
    plt.close()
...
def plot_genre_convergence_heatmap(convergence_matrix, save=True):
    ...
    if save:
        save_path = os.path.join(FIG_DIR, 'genre_convergence_heatmap.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Saved: {save_path}")
    plt.close()
...
def plot_cluster_genre_mapping(natural_labels, y_true, save=True):
    ...
    if save:
        save_path = os.path.join(FIG_DIR, 'cluster_genre_mapping.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Saved: {save_path}")
    plt.close()
    ...
    if save:
        save_path = os.path.join(FIG_DIR, 'cluster_genre_summary.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Saved: {save_path}")
    plt.close()
