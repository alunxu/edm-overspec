...
def plot_validation_figure(k_values, metrics, consensus_k, method_ks, out_path):
    ...
    line_specs = [
        ('silhouette',    'Silhouette ↑',         True,  '#7aa1f2', 'o'),
        ('inertia',       'Inertia (elbow) ↓',    False, '#5ec7a0', 's'),
        ('davies_bouldin','Davies-Bouldin ↓',      False, '#e8a14b', '^'),
        ('calinski',      'Calinski-Harabasz ↑',   True,  '#aa80f2', 'D'),
    ]
    ...
    # Consensus k line
    ax.axvline(x=17, color='#e87a7a', linestyle='--',
               linewidth=1.8, alpha=0.85, zorder=2,
               label='Consensus (17–20)')

    # Shade the consensus region lightly from 17 to 20
    ax.axvspan(17, 20,
