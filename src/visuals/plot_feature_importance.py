...
Panel (a): Stacked horizontal bar chart of the top-20 features (normalised
...
# Top 20 by total stacked importance
df['total_importance'] = df[score_cols].sum(axis=1)
top = df.sort_values('total_importance', ascending=False).head(20)
top = top.iloc[::-1]
...
ax_bar.set_title('(a) Top 20 Important Features', fontsize=28, fontweight='bold',
                 loc='left', pad=12)
