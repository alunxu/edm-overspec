...
def engineer_features(df):
    """Apply feature engineering consistent with earlier analysis"""
    # Get numeric columns
    numeric_cols = []
    ...
    for col in numeric_cols[:10]:
        if col in feature_df.columns:
            # Rolling statistics
            feature_df[f'{col}_roll_mean_5'] = feature_df[col].rolling(5, min_periods=1).mean()
            feature_df[f'{col}_roll_std_5'] = feature_df[col].rolling(5, min_periods=1).std()
