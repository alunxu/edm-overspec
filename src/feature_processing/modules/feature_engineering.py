...
class EDMFeatureEngineer:
    ...
    def _create_multi_scale_features(self, X):
        ...
        # 6. Engineered Tempogram Density (Fast vs Slow) & Peak Concentration
        fast_energy = pd.Series(0.0, index=X.index)
        slow_energy = pd.Series(0.0, index=X.index)
        total_energy = pd.Series(0.0, index=X.index)
        
        for i in range(1, 6):
            bpm_col_name = f'fourier_peak{i}_bpm'
            mean_col_name = f'fourier_peak{i}_mean'
            if bpm_col_name in X.columns and mean_col_name in X.columns:
                bpm_col = X[bpm_col_name]
                mean_col = X[mean_col_name]
                
                # Add to fast if BPM > 130
                fast_energy += np.where(bpm_col > 130, mean_col, 0)
                # Add to slow if BPM <= 100
                slow_energy += np.where(bpm_col <= 100, mean_col, 0)
                total_energy += mean_col
                
        X_multi['tempogram_fast_slow_ratio'] = fast_energy / (slow_energy + 1e-6)
        new_features.append('tempogram_fast_slow_ratio')
        
        if 'fourier_peak1_mean' in X.columns:
            # Concentration measures how dominant the first peak is vs the rest (4-on-the-floor vs scattered track)
            X_multi['fourier_peak_concentration'] = X['fourier_peak1_mean'] / (total_energy + 1e-6)
            new_features.append('fourier_peak_concentration')
