...
\begin{document}
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=\columnwidth]{figs/paper/tsne.png}
	...
	\textbf{MERT-95M} (768 dimensions): a music-specific transformer trained on 160M music clips using masked language modeling~\cite{li2024mert}. MERT learns hierarchical music representations without genre labels.

	\textbf{CLAP-tiny} (512 dimensions): a contrastive audio-language model trained on 630K audio-text pairs~\cite{wu2023clap}. CLAP's multimodal training provides a domain-agnostic perspective.
	...
	\begin{figure}[!t]
		\centering
		\includegraphics[width=0.95\columnwidth]{figs/paper/feature_importance.png}
	...
	We emphasize that this finding concerns \emph{acoustic} overspecification and does not imply that the commercial taxonomy is inherently flawed. Genre labels serve cultural, historical, and commercial functions that extend well beyond acoustic separability---they encode community identity, regional scene affiliations, DJ set-programming conventions, and listener expectations shaped by years of cultural practice~\cite{brackett2016}. Two genres may be acoustically near-identical yet culturally distinct, and such distinctions are legitimate from a music-industry perspective. Our claim is narrower: when evaluated strictly on the basis of audio features, approximately one-third of the prescribed 35 categories lack sufficient acoustic differentiation to form distinct clusters, suggesting that the current taxonomy subdivides what are, acoustically, coherent styles.

	\subsubsection{Cluster--Genre Correspondence}

	Figure~\ref{fig:confusion} presents the confusion matrix between the 35 commercial genre labels and the 35 discovered clusters (at $k$=35), revealing which genres merge under data-driven clustering. Several genre pairs consistently co-occur within the same clusters: Melodic House \& Techno shares clusters with Progressive House; Minimal--Deep Tech overlaps heavily with Tech House; and the three Trance variants (Main Floor, Raw--Deep--Hypnotic, Peak Time--Driving) fragment across shared clusters rather than forming distinct groups. These merges are musically intuitive---the genres in question share tempo ranges, rhythmic structures, and production aesthetics---suggesting that the acoustic overlap reflects genuine stylistic proximity rather than feature-set limitations. Based on this analysis, the strongest candidates for acoustic consolidation include: \{Melodic House \& Techno, Progressive House\}, \{Minimal--Deep Tech, Tech House\}, \{the three Trance variants\}, and \{Bass House, Bass--Club\}.

	Conversely, several genres maintain high cluster purity: Hard Techno concentrates 138 of its 100 sampled tracks into a single cluster, Amapiano forms a near-exclusive cluster (261 songs), and Trance (Main Floor) achieves 149 songs in its dominant cluster. These ``crystallized'' genres occupy distinct regions of feature space, confirming that our features \emph{can} resolve genres when genuine acoustic differences exist.

	\begin{figure*}[t]
		\centering
		\includegraphics[width=0.85\textwidth]{figs/paper/confusion_matrix.png}
		\caption{Confusion matrix between 35 commercial genre labels (rows) and 35 discovered clusters (columns). Cell values show song counts. High off-diagonal concentrations indicate genre pairs that merge under acoustic clustering---for example, Melodic House \& Techno and Progressive House share clusters 0 and 14, while the three Trance variants spread across clusters 14, 22, and 23. Genres with strong diagonal dominance (e.g., Hard Techno in cluster 7, Amapiano in cluster 22) maintain acoustic distinctiveness.}
		\label{fig:confusion}
	\end{figure*}

	\subsubsection{Genre Purity and Acoustic Extremity}

	Figure~\ref{fig:profiles} reveals that genre purity correlates with acoustic extremity. Pure clusters occupy feature space vertices---for example, clusters dominated by DJ Tools tracks (clusters 5, 26, and 18 in the $k$=35 solution) maximize Electronic features while minimizing others. We note that DJ Tools is not a traditional genre but a utility category containing loops, samples, and mixing elements for DJs; these tracks span diverse acoustic profiles, which is why they appear across multiple clusters. Despite this caveat, their extreme acoustic positioning confirms that production-specific content creates distinct signatures. Similarly, Ambient-dominated clusters prioritize Harmonic over Rhythmic features. These extremes---whether functional (DJ Tools) or artistic (Ambient)---demonstrate how specialized production constraints create acoustic signatures that resist genre hybridization.

	Conversely, mixed-genre clusters (11--20\% purity) display varied but less extreme profiles. Rather than converging to a uniform center, they represent distinct hybrid styles---Trap-Future (C30) extends on Electronic/Rhythmic axes while Melodic House (C32) balances multiple dimensions. This diversity suggests genre blending occurs through multiple pathways rather than a single ``neutral'' acoustic space, with each hybrid drawing differently from parent traditions.

	\begin{figure}[t]
		\centering
		\includegraphics[width=0.95\columnwidth]{figs/paper/musical_profile.png}
		\caption{Pure clusters occupy feature extremes, while mixed clusters show more moderate rankings. The profiles are derived by aggregating 159 audio features into six interpretable dimensions: Energy (RMS, loudness, amplitude), Danceability (groove, beat strength), Tempo (BPM), Harmonic (chroma, tonnetz, pitch features), Rhythmic (tempogram, onset, percussive, beat features), and Electronic (spectral, MFCC, timbre features). Features were z-score normalized before aggregation. Values shown are percentile rankings (0--100) indicating relative position within the distribution of all clusters. Profiles represent cluster means.}
		\label{fig:profiles}
	\end{figure}
	...
	\begin{figure*}[t]
		\centering
		\includegraphics[width=0.85\textwidth]{figs/paper/dendrograms.png}
	...
	\begin{figure*}[t]
		\centering
		\includegraphics[width=0.85\textwidth]{figs/paper/emergence_analysis.png}
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.95\columnwidth]{figs/paper/radar_recall.png}
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.95\columnwidth]{figs/paper/natural_k_validation.png}
